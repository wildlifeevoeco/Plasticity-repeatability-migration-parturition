...
############# Script 3: Determining date of snow melt and green-up across each herd's range ################
...
### Make new dataset for only calving range (after arriving on summer range)
dataPostMig<-subset(data,JDay>=EndMig)
...
### Loops through all years and populations, reads in the relevant day of snowmelt and peak IRG data and crops it to 
### the population's 99% MCP and gets percentile values for pixels reaching peak green-up for all years. 
t<-1
for(j in Pops){
  ## Subdata for all points (snow melt)
  subDataAll<-subset(data,Herd==j)
  ptsAll<-SpatialPoints(cbind(subDataAll$x,subDataAll$y),
                     proj4string=CRS("+proj=utm +zone=21 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
  MCPAll<-mcp(ptsAll,percent=99)
  MCPrpAll<-spTransform(MCPAll,CRSobj="+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
  ## Subdata for calving only (IRG)
  subDataIRG<-subset(dataPostMig,Herd==j)
  ptsIRG<-SpatialPoints(cbind(subDataIRG$x,subDataIRG$y),
                        proj4string=CRS("+proj=utm +zone=21 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"))
  MCPIRG<-mcp(ptsIRG,percent=99)
  MCPrpIRG<-spTransform(MCPIRG,CRSobj="+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
  w<-4
  for(i in years){
    NDSI<-raster("Inputs/Landcover/NDSI-snow-off.tif", band = w)
    NDSI2<-mask(NDSI,MCPrpAll)
    NDSI3<-crop(NDSI2,MCPrpAll)
    IRG<-raster(eval(paste(text="Inputs/Landcover/PeakIRG/PeakIRG",i,".tif",sep="")))
    IRG2<-mask(IRG,MCPIRG)
    IRG3<-crop(IRG2,MCPIRG)
...
saveRDS(phenolSnow, "Output/Phenology/SnowOffDates.RDS")
saveRDS(phenolGreen, "Output/Phenology/GreenUpDates.RDS")
...
write.csv(SnowData,"Output/Tables/SnowOffDatesByHerd.csv")
...
write.csv(GreenData,"Output/Tables/GreenUpDatesByHerd.csv")
...
saveRDS(phenol2, "Output/Phenology/medianDates.RDS")
...
saveRDS(BRN,"Output/DataForBNR.RDS")
